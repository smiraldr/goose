...
| [IO Intelligence](https://io.net/)                                          | Inference for open-weight models (DeepSeek, Qwen, Llama, GLM, Kimi, gpt-oss and more) through io.net's OpenAI-compatible API. Curated model list; every entry sets tool_choice to auto because the API default is none. | `IONET_API_KEY` |
